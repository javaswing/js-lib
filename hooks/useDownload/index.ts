...
import { downloadHandle } from "./utils/downloadHandle";
...
function useDownLoad(source: SourceType, options: OptionsType) {
    ...
    asyncRes
      .then(async (res) => {
        if (res instanceof Response) {
          let type = res.headers.get("Content-Type") || "text/plain";
          const ext = type.split("/").pop()!;
          let extension = ext === "plain" ? "txt" : ext;
  ...
  }, [loading, options.downloadExt, options.downloadFileName, source]);
